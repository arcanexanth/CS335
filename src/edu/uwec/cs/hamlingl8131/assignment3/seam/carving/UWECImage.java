...
public class UWECImage {
	...
	public UWECImage transpose(){
		...
		UWECImage imageRotate = new UWECImage(height, width);
		
		for(int j=0; j < height; j++){
			for(int i = 0; i < width; i++){
				imageRotate.im.setRGB(j, i, im.getRGB(i, j));
			}
		}
		return imageRotate;
	}

	public UWECImage removeSeam(int[] seam) {
		UWECImage image = new UWECImage(getWidth()-1, getHeight());
		
		for(int j = 0; j < getHeight(); j++){
			for(int i = 0; i < seam[j]; i++){
				image.im.setRGB(i, j, im.getRGB(i, j));
			}
			for(int i = seam[j]+1; i<getWidth(); i++){
				image.im.setRGB(i-1, j, im.getRGB(i, j));
			}
		}
		return image;
	}
